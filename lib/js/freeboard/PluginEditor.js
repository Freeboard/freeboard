				...
				switch (settingDef.type)
				{
					...
					default:
					{
						...
						if(settingDef.type == "calculated")
						{
							...
							{
								newSettings.settings[settingDef.name] = $(this).val();
								freeboard.emit("value_editor_changed",
												{name: settingDef.name, value: $(this).val()});
							});
							...
							var expectsType = settingDef.expected?settingDef.expected:undefined;

							valueEditor.createValueEditor(input, expectsType);

							var datasourceToolbox =
								$('<ul class="board-toolbar datasource-input-suffix"></ul>');

							if(settingDef.depends)
							{
								freeboard.on("value_editor_changed", function(event, data)
								{
									var value = data.value;
									if(settingDef.dependent_selector)
									{
										value = value + settingDef.dependent_selector;
									}
									if(data.name === settingDef.depends)
									{
										valueEditor.setDependentValue(input, value)
									}
								});
							}
							else
                            ...
                            $(valueCell).append(datasourceToolbox);
						}
						else
						{

ValueEditor = function(theFreeboardModel)
{
	...
	function _isTypeMatch(value, expectsType) {
		...
		case EXPECTED_TYPE.VALUE: return !_.isArray(value) && !_.isObject(value);
	...
	function _renderAutocompleteDropdown(element, expectsType)
	{
		...
		var dependentValue = $(element).data("dependent_value");

		var valueString = inputString;
		if(dependentValue)
		{
			valueString = dependentValue + inputString;
		}
		...
		_autocompleteFromDatasource(valueString, theFreeboardModel.datasources(), expectsType);
...
v						}
	...
	return {
		...
		setDependentValue : function(element, value)
		{
			element.data("dependent_value", value);
		},

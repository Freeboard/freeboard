ValueEditor = function(theFreeboardModel)
{
	...
	{
		var li = $('<li>' + option.value + '</li>').appendTo(dropdown)
			.mouseenter(function()
			{
			...
			.bind("freeboard-insertValue", function()
			{
				...
				if(!_.isUndefined(_autocompleteReplacementString))
				{
					...
					{
						$(element).replaceTextAt(replacementIndex,
							replacementIndex + _autocompleteReplacementString.length,
							optionValue);
					}
				}
				else

...
function FreeboardModel(datasourcePlugins, widgetPlugins, freeboardUI)
{
	...
	this.serialize = function()
	{
		...
		return {
			...
			columns     : freeboardUI.getGridColumnsForSave()
		};
	}

	this.deserialize = function(object, finishedCallback)
	{
		freeboardUI.setGridColumnsOnLoad(object["columns"]);
...
function FreeboardUI()
{
	...
	/*
	 * This field is set whenever the user manually adds or removes
	 * a column to the grid. If it is defined, it will be returned
	 * by getGridColumnsForSave in place of the calculated value.
	 * It is cleared by setGridColumnsOnLoad.
	 */
	var MANUAL_COLUMNS = undefined;
	...
	/*
	 * If the user has manually set the number of columns, return
	 * the last value it was set to. Otherwise, search through all
	 * the panes and return the maximum number of columns for
	 * which a grid layout has been defined.
	 */
	function getGridColumnsForSave()
	{
		if(MANUAL_COLUMNS)
		{
	...
	/*
	 * Set the current number of columns to the value specified
	 * for the loaded freeboard, or the maximum permitted by
	 * the current window.
	 */
	function setGridColumnsOnLoad(numCols)
	{
		MANUAL_COLUMNS = undefined;
	...
	return {
		...
		getGridColumnsForSave : function()
		{
			return getGridColumnsForSave();
		},
		setGridColumnsOnLoad : function(numCols)
		{
			setGridColumnsOnLoad(numCols);
		}
	}
}
